...
void MPU6050::calibrate() {
	...
	while(true) {
		...
		if (abs(tmp_gyroMean.X) > maxGyroError) {
			tmp_gyroOffset.X -= 1+(tmp_gyroMean.X / 10);
		...
		if (abs(tmp_gyroMean.Y) > maxGyroError) {
			tmp_gyroOffset.Y -=  1+(tmp_gyroMean.Y / 10);
		...
		if (abs(tmp_gyroMean.Z) > maxGyroError) {
			tmp_gyroOffset.Z -=  1+(tmp_gyroMean.Z / 10);
		...
		if (abs(tmp_accelMean.X) > maxAccelError) {
			tmp_accelOffset.X -=  1+(tmp_accelMean.X / 10);
		...
		if (abs(tmp_accelMean.Y) > maxAccelError) {
			tmp_accelOffset.Y -= 1+(tmp_accelMean.Y / 10);
		...
		if (abs(tmp_accelMean.Z) > maxAccelError) {
			tmp_accelOffset.Z -= 1+(tmp_accelMean.Z / 10);
